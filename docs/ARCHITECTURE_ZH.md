...
- 会话由 `chat-service` 持久化；REST 加载历史消息，WebSocket/STOMP 推送新消息和已读事件。
- WebSocket 断线时前端使用低频 REST 轮询降级，重连后补拉遗漏消息。
- 当前只支持纯文本消息，不支持图片、语音或文件。
...
- 使用 REST 管理会话和分页读取历史消息，使用 WebSocket/STOMP 实时推送新消息和已读事件。
- 当前使用单实例 Simple Broker；扩展为多实例前必须引入外部 broker 或跨实例消息分发。
...
前端不得直接访问内部服务端口，例如 `8092`、`8093`、`8094`、`8095`、`8096`。REST API 只通过 Gateway；`/ws-chat` 由同源 Nginx 入口代理到内部 chat-service，这是为避免将现有 Gateway MVC 整体迁移到 WebFlux 而保留的实时通信例外。
...
| `/ws-chat` | Nginx WebSocket 代理到 `chat-service` |
...
- 没有真实支付、退款、物流、结算。
- 聊天当前仅支持纯文本；WebSocket 使用单实例内存 broker。
- Kafka 依赖不代表已完成事件架构。
...
- 多实例聊天部署时升级为 RabbitMQ STOMP broker relay 或等价跨实例消息分发。
